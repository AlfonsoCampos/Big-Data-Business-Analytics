...
def getMDBResource (label, lang, endpoint):
	...
	if (lang):
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX foaf:\
		<http://xmlns.com/foaf/0.1/> SELECT ?s ?titulo WHERE { ?s rdfs:label \"" + label + "\"@" + lang + " . ?s dc:title ?titulo } "
	else:
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX foaf:\
		<http://xmlns.com/foaf/0.1/> SELECT ?s ?titulo WHERE { ?s rdfs:label \"" + label + "\" . ?s dc:title ?titulo } "
